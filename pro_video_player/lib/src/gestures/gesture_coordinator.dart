...
class GestureCoordinator {
  ...
  void _detectAndLockGesture(double absDeltaX, double absDeltaY, Size screenSize) {
    ...
    if (_pointerCount == 1) {
      ...
      if (absDeltaX > absDeltaY && absDeltaX > 10.0 && callbacks.enableSeekGesture) {
  ...
  /// Checks if position is in brightness gesture area (left side, above bottom).
  bool _isInBrightnessArea(Offset position, Size screenSize) {
    ...
    // Must be on left edge (within sideGestureAreaFraction)
    if (relativeX >= callbacks.sideGestureAreaFraction) return false;

    // Must be above bottom exclusion zone
    final bottomFraction = callbacks.bottomGestureExclusionHeight / screenSize.height;
    if (relativeY >= (1.0 - bottomFraction)) return false;

    return true;
  }

  /// Checks if position is in volume gesture area (right side, above bottom).
  bool _isInVolumeArea(Offset position, Size screenSize) {
    final relativeX = position.dx / screenSize.width;
    final relativeY = position.dy / screenSize.height;

    // Must be on right edge (within sideGestureAreaFraction from right)
    if (relativeX <= (1.0 - callbacks.sideGestureAreaFraction)) return false;
