#!/usr/bin/env bash
# macOS native coverage report script
# Extracts and displays coverage data from xcresult

set -e

FRAMEWORK_NAME="${1:-pro_video_player_macos.framework}"

echo "╔══════════════════════════════════════════════════════════════════════════╗"
echo "║              macOS NATIVE COVERAGE SUMMARY (Plugin Swift)                ║"
echo "╠══════════════════════════════════════════════════════════════════════════╣"

# Find the most recent xcresult, excluding known stale ones
XCRESULT=$(find ~/Library/Developer/Xcode/DerivedData/Runner-*/Logs/Test -name "*.xcresult" -type d 2>/dev/null | grep -v "catlquiwfbksltdosxclfqmxmohm" | sort -r | head -1)
